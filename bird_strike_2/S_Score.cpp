...
void judge_text_draw() {
	...
	Color text_cloor2 = { 255,255,255,alpha };
	DrawText(judge_text.c_str(), 702, 182, judge_text_size, text_cloor2);
...
void show_score() {
	...
	DrawText(std::to_string(score).c_str(), 50, 50, 50, { 255,255,255,125 });
	DrawText(std::to_string(score).c_str(), 48, 48, 50, BLACK);
	if (return_order_counter() > 0) {
		DrawText(std::to_string(return_order_counter()).c_str(), 722, 152, connected_crow_text_size, { 255,255,255,125 });
